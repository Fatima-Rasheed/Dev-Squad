      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '32px 48px 48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'start' }}>
        ...
        <div style={{ backgroundColor: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px', minHeight: '380px' }}>
          {product.images && product.images[0] ? (
            <img src={product.images[0]} alt={product.name} style={{ width: '100%', maxHeight: '360px', objectFit: 'contain' }} />
          ) : (
              ...
              {product.variants.map((variant) => (
                <button
                  ...
                  style={{
                    ...
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', outline: 'none',
                  }}
                >
                  <div style={{ width: '36px', height: '44px', border: '1px solid #ccc', borderRadius: '4px 4px 6px 6px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f9f9f9', fontSize: '0.55rem', color: '#666' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
            <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #ddd' }}>
              <button onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                style={{ width: '40px', height: '48px', border: 'none', background: 'none', cursor: 'pointer', fontSize: '1.2rem', color: '#333' }}>−</button>
            ...
            <button
              ...
              style={{ flex: 1, backgroundColor: '#1a1a1a', color: '#fff', border: 'none', height: '48px', fontSize: '0.82rem', letterSpacing: '0.12em', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', opacity: addingToCart ? 0.7 : 1 }}
            >
              <span>🛍️</span>
      ...
      <div style={{ backgroundColor: '#f5f5f5', padding: '48px 0' }}>
        <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '0 48px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '80px' }}>
          ...
          <div>
            ...
            <h4 style={{ fontFamily: 'Georgia, serif', fontSize: '1.1rem', color: '#1a1a1a', marginBottom: '12px', fontWeight: 'normal' }}>Ingredient</h4>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '32px' }}>
            {relatedProducts.slice(0, 3).map((p, index) => (
              <Link key={p._id} to={`/product/${p._id}`} style={{ textDecoration: 'none', color: 'inherit', textAlign: 'center' }}>
                <div style={{ backgroundColor: index === 2 ? '#f0f0f0' : 'transparent', height: '200px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}>
                  {p.images && p.images[0] ? (
                    <img src={p.images[0]} alt={p.name} style={{ maxHeight: '180px', maxWidth: '100%', objectFit: 'contain' }} />
                  ) : (
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '32px', marginBottom: '32px' }}>
          <div>
            ...
            {['Black Tea', 'Green Tea', 'White Tea', 'Herbal Tea', 'Matcha', 'Chai', 'Oolong', 'Rooibos'].map((item) => (
              <p key={item} style={{ fontSize: '0.8rem', color: '#999', marginBottom: '6px' }}>{item}</p>
            ))}
          </div>
          <div>
          ...
          <div>
            ...
            <p style={{ fontSize: '0.8rem', color: '#999', marginBottom: '6px' }}>📍 3 Falahi St, Posdaran Ave</p>
            <p style={{ fontSize: '0.8rem', color: '#999', marginBottom: '6px' }}>✉ Email: info@teahouse.com</p>
            <p style={{ fontSize: '0.8rem', color: '#999', marginBottom: '6px' }}>📞 Tel: +92-300-0000000</p>
          </div>
        </div>
